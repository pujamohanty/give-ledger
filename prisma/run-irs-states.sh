#!/usr/bin/env bash
# run-irs-states.sh
# Imports all 56 IRS BMF state files using chunked processing.
# Each chunk runs in its own Node process (1GB heap) to avoid OOM on large states.
# Files already downloaded to ~/.webcrawler/irs-bmf/ are reused (idempotent).
#
# Usage:  bash prisma/run-irs-states.sh
#         bash prisma/run-irs-states.sh 2>&1 | tee /tmp/irs-import-all.log

set -u
cd "$(dirname "$0")/.."

STATE_FILES=(
  eo_al eo_ak eo_az eo_ar eo_ca eo_co eo_ct eo_dc eo_de eo_fl
...
CHUNK_SIZE=25000   # rows per chunk — safe for 1GB heap on any state
...
echo "════════════════════════════════════════════════════════════"
echo "  IRS BMF Import — ${TOTAL} state files, ${CHUNK_SIZE} rows/chunk"
echo "════════════════════════════════════════════════════════════"
...
for KEY in "${STATE_FILES[@]}"; do
  ...
  # ── Split CSV into chunks using awk (O(n), preserves header) ───────────
  DATA_ROWS=$(( $(wc -l < "$CSV") - 1 ))
  CHUNKS=$(( (DATA_ROWS + CHUNK_SIZE - 1) / CHUNK_SIZE ))
  printf "%d rows → %d chunk(s)\n" "$DATA_ROWS" "$CHUNKS"

  # Remove old chunks for this state
  rm -f "${CHUNK_DIR}/${KEY}_chunk_"*.csv

  # awk splits: writes header + rows into numbered chunk files.
  # close() each chunk file before opening the next one to stay within
  # Windows/Git-Bash per-process open-file limits (avoids silent truncation
  # when CHUNKS > ~5 on large states like CA, TX, NY).
  ...
  # ── Import each chunk in its own Node process ───────────────────────────
  STATE_TOTAL=0
...
if [ "$ERRORS" -gt 0 ]; then
  echo "  Errors: ${ERRORS} chunk(s) failed — re-run to retry skipped chunks"
fi
echo "════════════════════════════════════════════════════════════"
