...
import {
  ...
  getFeaturedSectorIds, type Sector, type GrowthLevel,
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-2 mb-2">
        ...
        <ChevronRight
          className={`w-4 h-4 text-gray-400 shrink-0 mt-0.5 transition-transform duration-150 ${
            isSelected ? "rotate-90 text-violet-500" : ""
          }`}
        />
      </div>
...
function RoleCard({ role, sectorName }: { role: Sector["roles"][0]; sectorName: string }) {
          ...
          <div className="mt-3">
            ...
            <div className="flex flex-wrap gap-1.5">
              {role.aiTools.map((tool) => (
                <span
                  key={tool}
                  className="text-xs font-medium bg-gray-900 text-white px-2.5 py-1 rounded-full"
                >
                  {tool}
                </span>
              ))}
...
export default function CareerCompassClient({ bfsTrend, isCredentialed, hasAccount }: Props) {
  ...
  // Scroll to detail panel on mobile when a sector is selected
  useEffect(() => {
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              ...
              {
                label: "New US businesses/mo",
                value: bfsTrend.totalApplications
                  ? `${(bfsTrend.totalApplications / 1000).toFixed(0)}k`
                  : "470k",
              },
              {
                label: "Sector growth YoY",
                value: bfsTrend.yoyChange ? `+${bfsTrend.yoyChange}%` : "+4%",
              },
            ].map(({ label, value }) => (
              <div
                key={label}
      ...
      <div className="sticky top-[52px] z-20 bg-white border-b border-gray-200 shadow-sm">
        <div className="max-w-5xl mx-auto px-6 py-3 flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
          ...
          {!hasAccount ? (
            <Link
              href="/signup"
              className="text-xs font-semibold bg-emerald-700 text-white px-4 py-2 rounded-lg hover:bg-emerald-800 transition-colors whitespace-nowrap"
            >
            ...
            <Link
              href="/opportunities"
              className="text-xs font-semibold bg-violet-700 text-white px-4 py-2 rounded-lg hover:bg-violet-800 transition-colors whitespace-nowrap"
            >
            ...
            <Link
              ...
              className="inline-flex items-center gap-1.5 text-xs font-semibold bg-amber-500 text-white px-4 py-2 rounded-lg hover:bg-amber-600 transition-colors whitespace-nowrap"
            >
      ...
      <div className="max-w-5xl mx-auto px-6 py-8">
          ...
          <div className="mb-6 flex items-start gap-3 bg-blue-50 border border-blue-100 rounded-xl px-4 py-3">
            ...
            <div>
              <p className="text-sm font-semibold text-blue-900">
                Fastest-growing sectors for AI-augmented talent in {stateName}
              </p>
              <p className="text-xs text-blue-700 mt-0.5">
                Based on US Census Bureau Business Formation Statistics —{" "}
                {US_STATES.find((s) => s.code === selectedState)?.name} new business formations. Sectors marked "Popular in your state" have the highest relative growth.
              </p>
            </div>
          </div>
        )}
        ...
        <div className="mb-5 flex items-center gap-2 flex-wrap">
          ...
          {(["Very High", "High", "Growing", "Steady"] as GrowthLevel[]).map((level) => (
            <span
              key={level}
              className={`inline-flex items-center gap-1 text-[11px] font-semibold px-2 py-0.5 rounded-full border ${GROWTH_COLORS[level]}`}
            >
              <span className={`w-1.5 h-1.5 rounded-full ${GROWTH_DOT[level]}`} />
              {level}
            </span>
        ...
        {filteredSectors.length === 0 && (
          <div className="text-center py-16 text-gray-400">
            ...
            <button onClick={() => setSearchQuery("")} className="text-xs text-violet-600 hover:underline mt-1">
              Clear search
            </button>
          </div>
        )}
        ...
        {selectedSector && (
          <div
            ref={detailRef}
            className="mt-2 rounded-2xl border border-violet-200 bg-white shadow-lg overflow-hidden"
          >
            {/* Panel header */}
            <div className="bg-gradient-to-r from-violet-950 to-indigo-950 px-6 py-5">
              <div className="flex items-start justify-between gap-4">
                ...
                <button
                  ...
                  className="text-gray-400 hover:text-white mt-1 shrink-0"
                >
                  <X className="w-5 h-5" />
                </button>
              </div>
            ...
            <div className="px-6 py-4 bg-indigo-50 border-b border-indigo-100 flex items-start gap-3">
              ...
              <div>
                <p className="text-xs font-semibold text-indigo-900">
                  All roles below are AI-Augmented — no prior domain specialism required
                </p>
                <p className="text-xs text-indigo-700 mt-0.5">
                  ...
                  Your GiveLedger credential — earned through verified NGO work — demonstrates you can apply these tools to real-world tasks.
                </p>
              </div>
            </div>
            ...
            <div className="px-6 py-5">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="text-sm font-bold text-gray-900">
                  {selectedSector.roles.length} AI-Augmented Roles in {selectedSector.name}
                </h3>
              ...
              <div className="space-y-2">
                {selectedSector.roles.map((role) => (
                  <RoleCard key={role.title} role={role} sectorName={selectedSector.name} />
                ))}
              </div>
              ...
              <div className="mt-6 flex flex-col sm:flex-row items-stretch sm:items-center gap-3 pt-5 border-t border-gray-100">
                <div className="flex-1">
                  <p className="text-xs font-semibold text-gray-900 mb-0.5">Ready to apply your credential?</p>
                  <p className="text-[11px] text-gray-500">
                    Companies in {selectedSector.name} will be listed here when Phase 3 launches.
                    Earn your GiveLedger credential now so you&apos;re ready to apply.
                  </p>
                </div>
                <div className="flex gap-2 shrink-0">
                  <Link
                    href="/opportunities"
                    className="text-xs font-semibold bg-emerald-700 text-white px-4 py-2.5 rounded-lg hover:bg-emerald-800 transition-colors"
                  >
                    Find NGO roles →
                  </Link>
                  <Link
                    href={`/donor/training/${selectedSector.roles[0]?.trainingSlug ?? "marketing-workflows"}`}
                    className="text-xs font-semibold border border-violet-200 text-violet-700 bg-violet-50 px-4 py-2.5 rounded-lg hover:bg-violet-100 transition-colors"
                  >
                    Training hub →
                  </Link>
                </div>
          ...
          {[
            {
              ...
              title: "Work with an NGO",
              body: "Apply for a volunteer or paid role at a verified GiveLedger NGO. Your hours, skills, and outcomes are recorded on-chain.",
            },
            {
              step: "02",
              icon: <Sparkles className="w-5 h-5 text-violet-600" />,
              title: "Earn your credential",
              body: "When the NGO confirms your contribution, you receive a blockchain-verified GiveLedger credential — a real work record, not a self-reported claim.",
            },
            {
              step: "03",
              icon: <TrendingUp className="w-5 h-5 text-blue-600" />,
              title: "Apply to companies",
              body: "Your credential is recognised by for-profit companies hiring AI-augmented generalists. Phase 3 will list company profiles here for proactive applications.",
            },
        ...
        <p className="mt-6 text-center text-[11px] text-gray-400">
          ...
          <a
            href="https://www.census.gov/econ/bfs/"
            target="_blank"
